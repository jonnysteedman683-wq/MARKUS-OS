...
# Source diagram shipped with this repo (copied into the vault on init/link).
ARCH_DOC_SRC = Path(__file__).resolve().parent / "MARKUS-RUN-SYNC-architecture.html"
...
"""
    ...
    def ensure_architecture_link(self, arch_doc_rel: Optional[Path] = None) -> Path:
        """Copy the repo's architecture HTML into the vault (if present) and emit a link note.

        Mirrors AXIOM-VAULT: the diagram lives in the repo, is copied into the vault
        namespace, and a small wikilink note points at it from the run namespace.
        """
        arch_rel = arch_doc_rel or ARCH_DOC_REL
        # Copy the source diagram into the vault so Obsidian can render it.
        if ARCH_DOC_SRC.exists():
            try:
                dest = self.vault_path / arch_rel
                dest.parent.mkdir(parents=True, exist_ok=True)
                dest.write_text(ARCH_DOC_SRC.read_text(encoding="utf-8"), encoding="utf-8")
            except Exception as exc:  # noqa: BLE001 — diagram is non-critical
                logger.warning(f"[VaultSync] architecture diagram copy failed: {exc}")
        link_file = self.runs_dir / "ARCHITECTURE_LINK.md"
        md = (f"# 📐 MARKUS Run-Sync Architecture\n\n"
               f"- Diagram: [[{arch_rel.as_posix().replace('.html', '')}]]\n"
               f"- Schema: operator command center fields in `00_COMMAND_CENTER.md`\n"
